...
contract DigitalLandTitle is ERC721, ERC721Pausable, Ownable {
    ...
    // ---------- Events ----------
    event LandMinted(address indexed to, uint256 indexed tokenId, string landTitleHash);
    ...
    function getLandTitleHash(uint256 tokenId) external view returns (string memory) {
    ...
    function getLandTitleInfoByWallet(address wallet)
        external
        view
        returns (LandTitleInfo[] memory)
    {
        uint256 count = balanceOf(wallet);
        LandTitleInfo[] memory list = new LandTitleInfo[](count);

        uint256 idx = 0;
        // ไล่ดูทุก token ที่เคยออก (O(totalSupply)) — เหมาะกับการเรียกแบบ off-chain
        for (uint256 i = 0; i < _tokenIdTracker && idx < count; i++) {
            if (!_exists(i)) continue;                 // เผื่ออนาคตมี burn
            if (ownerOf(i) == wallet) {
                list[idx] = LandTitleInfo({tokenId: i, landTitleHash: _landTitleHash[i]});
                idx++;
            }
        }
        return list;
