...
contract DigitalLandTitle is ERC721URIStorage, ERC721Pausable, Ownable {
    ...
    function getLandTitleInfoByWallet(
        address wallet
    ) external view returns (LandTitleInfo[] memory) {
        uint256 ownedTokensCount = balanceOf(wallet);  // จำนวน NFT ที่ Wallet นั้นถือ
        LandTitleInfo[] memory landTitles = new LandTitleInfo[](ownedTokensCount);

        uint256 index = 0;
        for (uint256 i = 0; i < _tokenIdTracker; i++) {
            if (ownerOf(i) == wallet) {
                string memory landTitleHash = tokenURI(i);  // ดึงข้อมูล landTitleHash
                landTitles[index] = LandTitleInfo(i, landTitleHash);  // เก็บข้อมูล TokenID และ landTitleHash
                index++;
            }
        }

        return landTitles;  // ส่งกลับข้อมูลทั้งหมดในรูปแบบที่ต้องการ
    }
